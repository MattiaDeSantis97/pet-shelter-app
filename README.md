...
markdown_content = """# 🐾 Pet Shelter 2026
...
- **6+ Pagine Distinte:** Home, Lista Animali (con filtri), Dettaglio Animale, Dashboard Volontario, Dashboard Admin, Login, Chi Siamo e Contattaci.
- **Routing Dinamico:** Utilizzo di `react-router-dom` (HashRouter) per la navigazione fluida e gestione dei parametri URL per le pagine di dettaglio.
- **Paginazione e Filtri:** La lista animali supporta il filtraggio per specie e una paginazione fluida per migliorare la UX.

### 👤 Gestione Utenti e Ruoli
- **Autenticazione Simulata:** Sistema di login con persistenza dello stato.
- **Multi-Ruolo:**
  - **Adottante:** Può inviare richieste di adozione e visualizzare i dettagli.
  - **Volontario:** Gestione completa (CRUD) degli annunci con caricamento immagini.
  - **Admin:** Approvazione o rifiuto delle richieste con aggiornamento automatico dello stato dell'animale (Badge "Adottato").

### 🛠️ State Management & API
- **Redux Toolkit:** Gestione centralizzata di animali, adozioni e autenticazione.
- **Operazioni Asincrone:** Utilizzo di `createAsyncThunk` per gestire il caricamento dati.
- **Integrazione API:** Consumo di API esterne (Dog.ceo) e simulazione di API custom tramite `localStorage` per persistenza dati offline.

### 📝 Form Controllati
- **Validazione:** Almeno 4 form (Login, Contatti, Inserimento/Modifica, Richiesta Adozione) con gestione degli errori e feedback visivo.

---

## 💻 Tech Stack
- **Frontend:** React 19, Tailwind CSS (Design moderno & Responsive).
- **State:** Redux Toolkit, Redux Thunk.
- **Tools:** Vite, ESLint, Git/GitHub.
- **API:** Dog.ceo (External Content), LocalStorage (Persistence).
...
L'interfaccia utilizza un approccio **Glassmorphism** con overlay chiari e sfocatura dello sfondo per garantire la leggibilità su background fotografici immersivi. Ogni card e componente è stato ottimizzato per il contrasto cromatico e l'accessibilità.
...
**Mattia De Santis**
Studente presso EPICODE - Front-end Programming Module.
